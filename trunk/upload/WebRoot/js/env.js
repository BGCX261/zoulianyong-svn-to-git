...
var Request=new function()
{
    ...
    this.send=function(_url,_data,_processRequest,_chunnel,_asynchronous)
    {
        ...
        {
            Status.showErrInfo("ajax请求处理","<font color=red>由于目的为空，请求失败，请检查！</font>","5秒后该提示自动消失");
        ...
        if (instance==null)
        {
            Status.showErrInfo("ajax请求处理","<font color=red>浏览器不支持ajax，请检查！</font>","5秒后该提示自动消失");
            window.setTimeout("Status.setStatusShow(false)",5000);
            return;
        }  
            ...
            instance.onreadystatechange=function()
            {
                if (instance.readyState == 4) // 判断对象状态
                {
                    if (instance.status == 200) // 信息已经成功返回，开始处理信息
                    {
                        ...
                        {
                            Status.showErrInfo("ajax回调处理时发生错误：","<font color=red>"+e.message+"</font>","5秒后该提示自动消失");
                            window.setTimeout("Status.setStatusShow(false)",5000);
                        }
                    }
                    else
                    {
                        Status.showErrInfo("ajax请求处理","<font color=red>您所请求的页面有异常，请检查！</font>","5秒后该提示自动消失");
                        window.setTimeout("Status.setStatusShow(false)",5000);
                    }
                }
            }
