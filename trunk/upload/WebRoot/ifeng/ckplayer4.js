...
function ckstyle(){
	...
	var ck=new Object()
	ck.addProperty('logo','ckplayer/assets4/cklogo.png');//logo地址,如果不像显示就设置成空
	ck.addProperty('buffer','ckplayer/assets4/buffer.swf');//缓冲图标地址，推荐使用swf
	ck.addProperty('play_g','ckplayer/assets4/images_Play_out.png');//播放按钮普通状态图片
	ck.addProperty('play_m','ckplayer/assets4/images_Play_on.png');//播放按钮鼠标经过状态图片
	ck.addProperty('pause_g','ckplayer/assets4/images_Pause_out.png');//暂停按钮普通状态图片
	ck.addProperty('pause_m','ckplayer/assets4/images_Pause_on.png');//暂停按钮鼠标经过状态图片
	ck.addProperty('pause_c','ckplayer/assets4/images_Pause_Scgedyke.png');//中间暂停按钮图片
	ck.addProperty('sound_g','ckplayer/assets4/images_Sound_out.png');//静音按钮普通状态图片
	ck.addProperty('sound_m','ckplayer/assets4/images_Sound_on.png');//静音按钮鼠标经过状态图片
	ck.addProperty('mute_g','ckplayer/assets4/images_Mute_out.png');//取消静音按钮普通状态图片
	ck.addProperty('mute_m','ckplayer/assets4/images_Mute_on.png');//取消静音按钮鼠标经过状态图片
	ck.addProperty('full_g','ckplayer/assets4/images_Full_out.png');//全屏按钮普通状态图片
	ck.addProperty('full_m','ckplayer/assets4/images_Full_on.png');//全屏按钮鼠标经过状态图片
	ck.addProperty('general_g','ckplayer/assets4/images_General_out.png');//取消全屏按钮普通状态图片
	ck.addProperty('general_m','ckplayer/assets4/images_General_on.png');//取消全屏按钮鼠标经过状态图片
	ck.addProperty('volume_g','ckplayer/assets4/images_Volume_back.png');//音量调节底部图片
	ck.addProperty('volume_m','ckplayer/assets4/images_Volume_on.png');//音量调节亮部图片
	ck.addProperty('volume_s','ckplayer/assets4/images_Volume_Float.png');//音量调节按钮图片
	ck.addProperty('schedule_s','ckplayer/assets4/images_Schedule.png');//进度调节按钮图片
	ck.addProperty('bottom_g','ckplayer/assets4/images_buttom_bg.png');//底部背景图片
	ck.addProperty('schedule_bg','ckplayer/assets4/images_Schedule_bg.png');//加载进度背景图片
	ck.addProperty('schedule_load','ckplayer/assets4/images_Schedule_load.png');//加载进度背景图片
	ck.addProperty('schedule_play','ckplayer/assets4/images_Schedule_play.png');//加载进度背景图片
	ck.addProperty('logo_width',87);//logo的宽
	ck.addProperty('logo_height',20);//logo的高
	ck.addProperty('logo_align',2);//logo的对齐方式，0是左上，1是左下，2是右上，3是右下
	ck.addProperty('logo_x',95);//这里跟ckalign有直接关系，如果是ckalign=0/1 左边距离 2/3 是右边距离
	ck.addProperty('logo_y',10);//同上，ckalign=0/2 离上边距离，1/3 是离下边距离
	ck.addProperty('ct_height',28);//底部容器的高
	ck.addProperty('play_left',0);//播放/暂停按钮离左边的距离
	ck.addProperty('clock_color',0x8FBCC2);//已播放时间/总时间的颜色
	ck.addProperty('clock_left',75);//已播放时间/总时间离左边的距离
	ck.addProperty('clock_top',4);//播放时间离底部容器上边的距离
	ck.addProperty('full_right',31);//全屏/取消全屏按钮离右边的距离,至少设置成按钮的宽度
	ck.addProperty('volume_right',100);//音量调节框离右边的距离
	ck.addProperty('volume_top',12);//音量调节框离底部容器顶端的距离
	ck.addProperty('volume_width',55);//音量调节框的宽
	ck.addProperty('volume_height',6);//音量调节框的高
	ck.addProperty('vs_width',11);//音量调节滑块的宽
	ck.addProperty('vs_height',11);//音量调节滑块框的高
	ck.addProperty('sound_right',140);//静音/取消静音离右边的距离
	ck.addProperty('pgb_height',6);//进度框的高度
	ck.addProperty('pgb_left',0);//进度框离左边的距离
	ck.addProperty('pgb_right',0);//进度框离右边的距离
	ck.addProperty('pgb_bottom',34);//进度框顶部离下面的的距离
	ck.addProperty('ss_width',12);//进度调节滑块的宽
	ck.addProperty('ss_height',12);//进度调节滑块框的高
	ck.addProperty('video_bottom',34);//播放器离底部的距离
