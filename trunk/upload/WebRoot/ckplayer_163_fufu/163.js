...
function ckstyle(){
	var ck=new Object();
	ck.mylogo='163/mylogo.png';//开始加载的logo,如果为空就不加载
	ck.logo='163/logo.swf,2,320,8';//logo地址,对齐方式(0/1/2/3),x,y,如果不想显示地址设置成空
	ck.buffer='163/buffer.swf';//缓冲图标地址，推荐使用swf
	ck.play_g='163/play_out.png';//播放按钮普通状态图片
	ck.play_m='163/play_on.png';//播放按钮鼠标经过状态图片
	ck.pause_g='163/pause_out.png';//暂停按钮普通状态图片
	ck.pause_m='163/pause_on.png';//暂停按钮鼠标经过状态图片
	ck.pause_c='163/pause_scgedyke.swf';//中间暂停按钮图片
	ck.sound_g='163/sound_out.png';//静音按钮普通状态图片
	ck.sound_m='163/sound_on.png';//静音按钮鼠标经过状态图片
	ck.mute_g='163/mute_out.png';//取消静音按钮普通状态图片
	ck.mute_m='163/mute_on.png';//取消静音按钮鼠标经过状态图片
	ck.full_g='163/full_out.png';//全屏按钮普通状态图片
	ck.full_m='163/full_on.png';//全屏按钮鼠标经过状态图片
	ck.general_g='163/general_out.png';//取消全屏按钮普通状态图片
	ck.general_m='163/general_on.png';//取消全屏按钮鼠标经过状态图片
	ck.volume_g='163/volume_back.png';//音量调节底部图片
	ck.volume_m='163/volume_on.png';//音量调节亮部图片
	ck.volume_s='163/schedule.png';//音量调节按钮图片
	ck.schedule_s='163/schedule.png';//进度调节按钮图片
	ck.bottom_g='163/buttom_bg.png';//底部背景图片
	ck.schedule_bg='163/schedule_bg.png';//加载进度背景图片
	ck.schedule_load='163/schedule_load.png';//加载进度背景图片
	ck.schedule_play='163/schedule_play.png';//加载进度背景图片
	ck.advmu='163/v_off.png';//前置广告静音按钮图片
	ck.advnomu='163/v_on.png';//前置广告取消静音按钮图片
	ck.advjump='163/adv_skip.png';//前置广告跳过广告图片
	ck.advmclose='163/close_adv.png';//关闭滚动广告的按钮
	ck.ctbar='31,0,0,31,0,2000';//底部控制栏的高，左边的距离，右边的距离，下边的距离,隐藏方式(0是不隐藏，1是全屏时隐藏，2是都隐藏),隐藏延迟时间
	ck.play_left=0;//播放/暂停按钮离左边的距离
	ck.clock='0x666666,70,55,5';//已播放时间/总时间的颜色,宽度,离左边的距离，离上边的距离
	ck.full_right=45;//全屏/取消全屏按钮离右边的距离,至少设置成按钮的宽度
	ck.vol='35,31,90,0,5,31';//音量调节框宽，高，离右边距离，上方距离，滑块的宽，高	
	ck.sound_right=102;//静音/取消静音离右边的距离
	ck.pgb='5,0,0,36,5,5';//进度框的高，离左边距离，离右边距离，离下面距离，滑块的宽，高
	ck.video_bottom=36;//视频区域底部离播放器底部的距离
	ck.re_buttom=31;//**底部距离参考值，该值很重要，用来计算如暂停广告，悬浮按钮等的坐标值，正常设置成控制栏或进度调节框离底部的距离
	ck.video_bo='100,1';//视频缓冲时间，是否采用图形优化
	ck.pa='0,80'//浮动暂停按钮是否固定，=0在有广告时左下角对齐，1表示固定中间,浮动暂停按钮的透明度
	ck.buffer_wh='45,45';//缓冲图标的宽,高
	...
	ck.advmarquee='1,0x000000,20,5,20,56,3,50';//是否显示底部广告(0/1),背景颜色,高度,左边的距离,右边的距离,离下面的距离,滚动的距离(0静止),间隙时间(越小越快,建议不小于20),如果开启广告需要有function ckmarqueeadv(){return '广告内容'}
	ck.advms='1,3,15,52';//滚动广告是否显示关闭按钮(0/1),位置(0/1/2/3),x,y
